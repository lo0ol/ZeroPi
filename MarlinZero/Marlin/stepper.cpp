...
STEPPER_TIMER_ISR()//ISR(TIMER1_COMPA_vect)
{
    ...
    if (current_block != NULL) {
      ...
      step_events_completed = 0;
        ...
        if(current_block->steps_z > 0) {
          enable_z();
          STEPPER_TIMER_OCR = 2000; //1ms wait
          return;
        }
      #endif
...
//      #endif
    ...
    if (step_events_completed <= (unsigned long int)current_block->accelerate_until) {
      ...
      STEPPER_TIMER_OCR = OCR1A_nominal;
...
void st_init()
{
  ...
  initStepperTimer();
  stepperSetResolution(0,16);
  stepperSetResolution(1,16);
  stepperSetResolution(2,16);
  stepperSetResolution(3,16);
